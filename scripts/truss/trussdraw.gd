...
enum Mode { DRAW_MEMBERS, ADD_SUPPORTS, ADD_FORCES, SOLVED, RULER}
...
func _draw() -> void:
	...
	for member in line_data:
		...
		if force_label != "":
			var mid = (member.start + member.end) / 2
			var normal = (member.end - member.start).rotated(PI/2).normalized()
			draw_string(font, mid + normal * 20, force_label, HORIZONTAL_ALIGNMENT_CENTER, -1, font_size, golden)
	...
	if current_mode == Mode.SOLVED:
		for node_pos in reaction_forces:
			draw_string(font, node_pos + Vector2(0, 50), reaction_forces[node_pos], HORIZONTAL_ALIGNMENT_CENTER, -1, font_size, golden)

	if current_mode == Mode.RULER and is_measuring:
		var d = end_point - start_point
		var mid = (start_point + end_point) / 2
		var a = str(abs(round(rad_to_deg(d.angle())))) + "°"
		draw_line(start_point, end_point, golden, 2)
		draw_line(start_point, Vector2(end_point.x, start_point.y), Color(1, 1, 1, 0.4))
		draw_line(Vector2(end_point.x, start_point.y), end_point, Color(1, 1, 1, 0.4))
		draw_string(font, start_point + Vector2(10, -10), a, HORIZONTAL_ALIGNMENT_LEFT, -1, 14, golden)
		draw_string(font, end_point + Vector2(10, 20), a, HORIZONTAL_ALIGNMENT_LEFT, -1, 14, golden)
		draw_string(font, mid, str(int(d.length())), HORIZONTAL_ALIGNMENT_LEFT, -1, 14, golden)
		draw_string(font, Vector2(mid.x, start_point.y - 5), str(int(abs(d.x))), HORIZONTAL_ALIGNMENT_CENTER, -1, 12, golden)
		draw_string(font, Vector2(end_point.x + 5, mid.y), str(int(abs(d.y))), HORIZONTAL_ALIGNMENT_LEFT, -1, 12, golden)
...
func handle_ruler_logic(event):
	...
	if event is InputEventMouseMotion and is_measuring:
...
func draw_support_icon(pos: Vector2, type: SupportType):
	...
	match type:
		...
		SupportType.PIN_Y_NEG: 
			tex = config.pin_texture; 
			angle = PI/2; manual_offset = config.pin_hole_offset
