...
func handle_member_drawing(event: InputEvent):
	var global_mouse = host.get_global_mouse_position()
	
	if event is InputEventMouseButton and event.pressed:
		if host.is_grabbing:
			if event.button_index == MOUSE_BUTTON_LEFT:
				host.update_grabbed_point(host.best_pos(global_mouse))
		...
		if event.button_index == MOUSE_BUTTON_LEFT:
			if not host.is_drawing:
				host.start_point = host.best_pos(global_mouse)
				host.is_drawing = true
			else:
				host.end_point = host.best_pos(global_mouse)
			...
			if host.is_drawing:
				...
				host.line_data = host.line_data.filter(func(member):
					return !member.has_point(host.best_pos(global_mouse))
				)
			host.renderer.queue_redraw()

	if event is InputEventKey and event.pressed and event.keycode == KEY_G:
		var snapped_pos = host.best_pos(global_mouse)
		if global_mouse.distance_to(snapped_pos) < host.config.snap_radius:
			host.is_drawing = false
	...
	if host.is_grabbing and event is InputEventMouseMotion:
		...
		for member in host.line_data:
			if member.start != host.grabbed_point and global_mouse.distance_to(member.start) < host.config.snap_radius:
				snap_pos = member.start
				break
			if member.end != host.grabbed_point and global_mouse.distance_to(member.end) < host.config.snap_radius:
				snap_pos = member.end
				break
		host.update_grabbed_point(snap_pos)
		host.renderer.queue_redraw()

func handle_support_logic(event: InputEvent):
	var global_mouse = host.get_global_mouse_position()
	if event is InputEventMouseButton and event.pressed:
		var pos = host.best_pos(global_mouse)
		if global_mouse.distance_to(pos) < host.config.snap_radius:
			if event.button_index == MOUSE_BUTTON_LEFT:
				var current = host.node_supports.get(pos, 0)
				host.node_supports[pos] = (current + 1) % 9
			elif event.button_index == MOUSE_BUTTON_RIGHT:
				host.node_supports.erase(pos)
		host.renderer.queue_redraw()

func handle_force_logic(event: InputEvent):
	var global_mouse = host.get_global_mouse_position()
	if event is InputEventMouseButton:
		var snapped_node = host.best_pos(global_mouse)
		if event.button_index == MOUSE_BUTTON_LEFT:
			if event.pressed:
				...
				if host.is_drawing_forces:
					var target_pos = host.best_pos(global_mouse)
			...
			if global_mouse.distance_to(snapped_node) < host.config.snap_radius:
