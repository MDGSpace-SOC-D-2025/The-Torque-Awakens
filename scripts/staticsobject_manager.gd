	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			if is_grabbing:
				...
				selected_object = _get_object_at(mouse_pos)
...
func _remove_object_at(pos: Vector2):
	...
	if to_remove != -1:
		main.objects[to_remove].body.queue_free()
		main.objects.remove_at(to_remove)
